...
constexpr int64_t heartbeat_interval_ms = 5 * 60 * 1000;
}
...
static std::string utc_timestamp_iso() {
...
static void gateway_heartbeat_loop(const RuntimeOptions& opts, MqttClient& mqtt) {
    const std::string topic = "gateway/position";

    while (g_running) {
        std::string json = "{\"gatewayId\":\"" + opts.gateway_id
            + "\",\"timestamp\":\"" + utc_timestamp_iso()
            + "\",\"lon\":null,\"lat\":null,\"heading\":null,\"driveBy\":false}";

        mqtt.publish_json(topic, json);
        LOG_INFO("Gateway heartbeat sent to %s", topic.c_str());

        // Sleep in short intervals to allow quick shutdown
        for (int64_t waited = 0; waited < heartbeat_interval_ms && g_running; waited += 250) {
...
int main(int argc, char* argv[]) {
    ...
    std::thread heartbeat_thread(gateway_heartbeat_loop, std::cref(opts), std::ref(mqtt));
